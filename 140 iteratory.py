class Combinations:
    ...
    def __next__(self):
        ...
        if self.current_product >= len(self.products):
            self.current_product =0
            raise StopIteration()   # wychwytywane prze `for` ale nie przez `next(it)`
    ...
    # iterable - zwraca obiekt z metodą next, żeby dało sie użyć `for x in Combinations`
    # bez tego, iterować można tylko przez `next(it)`
    def __iter__(self):
        return  self
